...
namespace Micro
{
    public class TetrisClient
    {
        ...
        private readonly ILogger<TetrisClient> _logger;

        public TetrisClient(HttpClient client, ILogger<TetrisClient> logger)
        {
            this.client = client;
            this._logger = logger;
        }
        ...
        /// <summary>
        /// Consumes the individual microservice based off of a given id.  Set up by Chris and Noah.
        /// </summary>
        ...
        public async Task<GameInfo[]> GetGameByIdAsync(int id)
        {
            try
            {
                var responseMessage = await client.GetAsync("/Micro"); // Adjust the endpoint URL to include the ID
